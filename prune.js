...
function pruneFromSolved() {
  ...
  for (var i = 0; i < draaien.length; i++) {
    ...
    if (!pruningtable[Coord[0]][Coord[1]][Coord[2]]) {
      pruningtable[Coord[0]][Coord[1]][Coord[2]] = inverse(draaien[i])
    }
    oplosstaat()
  }
  ...
  for (var i = 0; i < draaien.length; i++) {
    for (var j = 0; j < draaien.length; j++) {
      ...
      draai(draaien[j])
      if (!pruningtable[Coord[0]][Coord[1]][Coord[2]]) {
        pruningtable[Coord[0]][Coord[1]][Coord[2]] = inverse(draaien[j]) + " " + inverse(draaien[i])
      }
      oplosstaat()
    }
  }
  ...
  for (var i = 0; i < draaien.length; i++) {
    for (var j = 0; j < draaien.length; j++) {
      for (var k = 0; k < draaien.length; k++) {
        ...
        draai(draaien[k])
        if (!pruningtable[Coord[0]][Coord[1]][Coord[2]]) {
          pruningtable[Coord[0]][Coord[1]][Coord[2]] = inverse(draaien[k]) + " " + inverse(draaien[j]) + " " + inverse(draaien[i])
        }
        oplosstaat()
      }
    }
    ...
    for (var j = 0; j < draaien.length; j++) {
      for (var k = 0; k < draaien.length; k++) {
        for (var l = 0; l < draaien.length; l++) {
          ...
          draai(draaien[l])
          if (!pruningtable[Coord[0]][Coord[1]][Coord[2]]) {
            pruningtable[Coord[0]][Coord[1]][Coord[2]] = inverse(draaien[l]) + " " + inverse(draaien[k]) + " " + inverse(draaien[j]) + " " + inverse(draaien[i])
          }
          oplosstaat()
        }
      }
